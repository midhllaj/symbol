...
file_path = "d:/projects/symbol-main/about.html"
start_marker = '<div class="mod-client-works__logo-grid delay-20 animation animation-mobile">'
end_marker = '</div>'
...
for filename in files:
    ...
    new_content += f"""                                  <div class="logo-item">
...
new_content += '                                  <div class="logo-item logo-item--empty tablet-only"></div>\n'
new_content += '                                  <div class="logo-item logo-item--empty tablet-only"></div>\n'
new_content += '                                  <div class="logo-item logo-item--empty mobile-only"></div>\n'
...
# Finding the specific block
for i, line in enumerate(lines):
    if 'class="mod-client-works__logo-grid' in line:
        start_index = i
        break

if start_index != -1:
    # Find the matching closing div. It's indentation based in this well-formatted file.
    # The start line has indentation.
    start_line = lines[start_index]
    indentation = start_line[:start_line.find('<')]
    
    # We look for the closing div with the same indentation
    for i in range(start_index + 1, len(lines)):
        if lines[i].startswith(indentation + '</div>'):
            end_index = i
            break

if start_index != -1 and end_index != -1:
    # Keep the start line and end line, replace everything in between
    output_lines = lines[:start_index+1]
    output_lines.append(new_content)
    output_lines.extend(lines[end_index:])
    
    with open(file_path, 'w', encoding='utf-8') as f:
        f.writelines(output_lines)
    print("Successfully replaced logos.")
else:
    print("Could not find the start or end markers.")
